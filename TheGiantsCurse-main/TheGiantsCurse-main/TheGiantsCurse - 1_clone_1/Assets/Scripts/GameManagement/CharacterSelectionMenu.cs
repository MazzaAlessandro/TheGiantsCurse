...
public class CharacterSelectionMenu : NetworkBehaviour
{
    ...
    public override void OnNetworkSpawn()
    {
        if (IsClient)
        {
            Characters[] allCharacters = characterDatabase.GetAllCharacters();
        ...
        if (IsHost)
        {
            joinCodeText.text = "Code to join: " + ServerManager.instance.joinCode;
        }
    }
    ...
    public void Select(Characters character)
    {
        for (int i = 0; i < players.Count; i++)
        {
        ...
        introInstance = Instantiate(character.IntroPrefab, new Vector3(0f, 0f, -7f), Quaternion.identity);
        introInstance.transform.Rotate(new Vector3(0, 180, 0));
